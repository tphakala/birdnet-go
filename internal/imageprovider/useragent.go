...
// belongs in a neutral file. The construction and memoization themselves now
// live in internal/useragent, shared with internal/guideprovider, which is the
// other package that talks to Wikimedia; the wrappers here keep this package's
// call sites and their documented policy constraints unchanged.
...
const (
	...
	// useragent.BuildProduct) so forks identify themselves rather than the
	// upstream repo.
	//
	...
	userAgentName    = useragent.ProductName
	userAgentLibrary = useragent.LibraryToken
)
...
func currentAppVersion() string { return useragent.AppVersion() }

// buildUserAgent constructs a user-agent string that complies with Wikimedia's
// robot policy.
func buildUserAgent(appVersion string) string { return useragent.BuildProduct(appVersion) }
...
func appUserAgent() string { return useragent.Product() }
