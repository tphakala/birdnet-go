...
func (m *MySQLManager) Initialize() error {
	...
	if err != nil {
		reportInitFailure("mysql", "AutoMigrate", err, m.config.Host, m.config.Database)
	...
	if err := m.db.FirstOrCreate(&state, entities.MigrationState{ID: 1}).Error; err != nil {
		reportInitFailure("mysql", "initMigrationState", err, m.config.Host, m.config.Database)
		return fmt.Errorf("failed to initialize migration state: %w", err)
	}

	// Seed lookup tables
	if err := m.seedLookupTables(); err != nil {
		reportInitFailure("mysql", "seedLookupTables", err, m.config.Host, m.config.Database)
		return fmt.Errorf("failed to seed lookup tables: %w", err)
	}

	// Seed default AI model (BirdNET)
	if err := m.seedDefaultModel(); err != nil {
		reportInitFailure("mysql", "seedDefaultModel", err, m.config.Host, m.config.Database)
		return err
	}
	return nil
}
