...
func TestRangeFilterCheck(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "no acoustic model loaded is not applicable (N=0)",
			info:       RangeFilterStatusInfo{ClassifierLoaded: false, LocationConfigured: true, Active: false},
			wantStatus: health.StatusHealthy,
		},
		{
			name:       "no location configured is not applicable",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: false, Active: false},
			wantStatus: health.StatusHealthy,
		},
		{
			name:       "location configured but no filter active is fail-open",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: true, Active: false},
			wantStatus: health.StatusCritical,
		},
		{
			name:       "fell back to embedded filter warns",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: true, Active: true, FellBack: true},
			wantStatus: health.StatusWarning,
		},
		{
			name:       "geomodel mapped zero species warns",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: true, Active: true, GeomodelActive: true, MappedSpecies: 0},
			wantStatus: health.StatusWarning,
		},
		{
			name:       "active geomodel with mapped species is healthy",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: true, Active: true, GeomodelActive: true, MappedSpecies: 6500},
			wantStatus: health.StatusHealthy,
		},
		{
			name:       "active embedded filter is healthy",
			info:       RangeFilterStatusInfo{ClassifierLoaded: true, LocationConfigured: true, Active: true},
			wantStatus: health.StatusHealthy,
		},
	}
