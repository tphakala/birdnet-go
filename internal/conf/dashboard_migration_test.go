	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectMigrated {
				...
				assert.Equal(t, "recent-hearing", elements[2].Type)
				assert.True(t, elements[2].Enabled)
				assert.Equal(t, "live-spectrogram", elements[3].Type)
				assert.True(t, elements[3].Enabled)
				assert.Equal(t, "detections-grid", elements[4].Type)
				assert.True(t, elements[4].Enabled)

				// Verify current activity tiles share a row by default
				assert.Equal(t, "half", elements[1].Width)
				assert.Equal(t, "half", elements[2].Width)
				...
				assert.Equal(t, "recent-hearing-0", elements[2].ID)
				assert.Equal(t, "live-spectrogram-0", elements[3].ID)
				assert.Equal(t, "detections-grid-0", elements[4].ID)
...
func TestSettings_MigrateDashboardLayout_Idempotent(t *testing.T) {
	...
	assert.Len(t, settings.Realtime.Dashboard.Layout.Elements, 5)
