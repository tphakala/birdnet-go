...
// TestPerchPredictWithEmbeddings_SuccessRecordsExactlyOnce verifies the embedding
// predict path records exactly one success under the Perch model label. The
// birdnet.predict_embeddings span shares the prediction metric with the plain
// predict span, so PredictWithEmbeddings stays at telemetry parity with Predict.
func TestPerchPredictWithEmbeddings_SuccessRecordsExactlyOnce(t *testing.T) {
	resetGlobalMetrics(t)
	t.Cleanup(func() { resetGlobalMetrics(t) })
	m := newTestMetrics(t)
	SetMetrics(m)
	...
	results, embedding, err := p.PredictWithEmbeddings(t.Context(), [][]float32{{0.1, 0.2, 0.3}})
	require.NoError(t, err)
	require.NotEmpty(t, results)
	...
	assert.InDelta(t, 1.0, predictionCount(t, m, RegistryIDPerchV2, "success"), 0,
		"a successful Perch embedding prediction must be recorded exactly once")
	assert.InDelta(t, 0.0, predictionCount(t, m, RegistryIDPerchV2, "error"), 0,
		"a successful Perch embedding prediction must not record an error")
}

// TestPerchPredictWithEmbeddings_EmptySampleRecordsNothing verifies the
// empty-sample guard on the embedding path tags the span errored but records no
// prediction, mirroring Perch.Predict's pre-inference guard semantics.
func TestPerchPredictWithEmbeddings_EmptySampleRecordsNothing(t *testing.T) {
	resetGlobalMetrics(t)
	t.Cleanup(func() { resetGlobalMetrics(t) })
	m := newTestMetrics(t)
	SetMetrics(m)
	...
	_, _, err := p.PredictWithEmbeddings(t.Context(), nil)
	require.Error(t, err)
...
// TestPerchPredictWithEmbeddings_LabelMismatchRecordsErrorOnce verifies a
// label/score mismatch (a failure after inference begins) on the embedding path
// records exactly one error.
func TestPerchPredictWithEmbeddings_LabelMismatchRecordsErrorOnce(t *testing.T) {
	resetGlobalMetrics(t)
	t.Cleanup(func() { resetGlobalMetrics(t) })
	m := newTestMetrics(t)
	SetMetrics(m)
	...
	_, _, err := p.PredictWithEmbeddings(t.Context(), [][]float32{{0.1, 0.2}})
	require.Error(t, err)
...
func (f *batEmbeddingExtractor) EmbeddingDim() int { return len(f.embeddings) }
