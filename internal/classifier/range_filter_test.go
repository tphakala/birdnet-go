...
// 12:00, instead of the old time.Now().Truncate(24h) which rounds to UTC
// midnight and reports the UTC calendar day near the local day boundary.
func TestLocalNoon(t *testing.T) {
	t.Parallel()
	t.Attr("component", "classifier")
	t.Attr("type", "unit")
	t.Attr("feature", "range-filter")
	...
	// for the geomodel week lookup than the local one.
	tests := []struct {
		name    string
		now     time.Time
		wantDay int
