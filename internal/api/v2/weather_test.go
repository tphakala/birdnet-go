...
func TestGetDailyWeather(t *testing.T) {
	...
	if assert.NoError(t, controller.GetDailyWeather(c)) {
		...
		assert.Equal(t, time.Unix(1672559400, 0).In(time.Local), response.Sunrise)
		assert.Equal(t, time.Unix(1672594800, 0).In(time.Local), response.Sunset)
...
func TestGetWeatherForDetection(t *testing.T) {
	...
	if assert.NoError(t, controller.GetWeatherForDetection(c)) {
		...
		assert.Equal(t, time.Unix(1672559400, 0).In(time.Local), response.Daily.Sunrise)
		assert.Equal(t, time.Unix(1672594800, 0).In(time.Local), response.Daily.Sunset)
