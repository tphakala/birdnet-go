...
func TestConvertSpeciesScoresNoNames(t *testing.T) {
	...
	require.NotNil(t, got[0].IsManuallyIncluded)
	...
	assert.Nil(t, got[1].IsManuallyIncluded)
...
func TestNativeSpeciesScores(t *testing.T) {
	...
	scores := []classifier.SpeciesScore{
		...
		{Label: "Amazona viridigenalis_Red-crowned Amazon", Score: 0.95, IsManuallyIncluded: true},
	...
	assert.True(t, got[0].IsManuallyIncluded, "native provenance must survive raw-score filtering")
