...
func TestDedupeSpeciesForDisplay(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			in: []dto.RangeFilterSpecies{
				{Label: "Amazona viridigenalis_Red-crowned Parrot", ScientificName: "Amazona viridigenalis", CommonName: "Red-crowned Amazon", Score: new(1.0), IsManuallyIncluded: new(true), IsSyntheticOverride: true},
			...
			want: []dto.RangeFilterSpecies{
				{Label: "Amazona viridigenalis_Red-crowned Parrot", ScientificName: "Amazona viridigenalis", CommonName: "Red-crowned Amazon", Score: new(1.0), RangeScore: new(0.95), IsManuallyIncluded: new(true), IsSyntheticOverride: true},
				{Label: "Corvus brachyrhynchos_American Crow", ScientificName: "Corvus brachyrhynchos", CommonName: "American Crow", Score: new(0.99)},
			},
		},
		{
		...
		},
		{
			...
			in: []dto.RangeFilterSpecies{
				{Label: "Parus major_Talitiainen", ScientificName: "Parus major", CommonName: "Great Tit", Score: new(1.0), HasCustomConfig: new(true), IsSyntheticOverride: true},
				{Label: "Parus major_Great Tit", ScientificName: "Parus major", CommonName: "Great Tit", Score: new(0.73)},
			},
			want: []dto.RangeFilterSpecies{
				{Label: "Parus major_Talitiainen", ScientificName: "Parus major", CommonName: "Great Tit", Score: new(1.0), RangeScore: new(0.73), HasCustomConfig: new(true), IsSyntheticOverride: true},
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := range tt.want {
				...
				assert.Equal(t, tt.want[i].HasCustomConfig, got[i].HasCustomConfig,
					"row %d custom config", i)
...
func TestScoreHigher(t *testing.T) {
	t.Parallel()
	assert.True(t, scoreHigher(new(1.0), new(0.5)))
	assert.False(t, scoreHigher(new(0.5), new(1.0)))
	assert.False(t, scoreHigher(new(0.5), new(0.5)))
	// nil score sorts below any real score.
	assert.False(t, scoreHigher(nil, new(0.0)))
	assert.True(t, scoreHigher(new(0.0), nil))
	assert.False(t, scoreHigher(nil, nil))
}
