...
// guideEnabledSettings returns settings for an enabled species guide, mirroring the
// production defaults in conf/defaults.go: the show* sub-features default to true, so
// a zero-valued struct with only Enabled set represents no real configuration. The
// handlers gate on these flags (see requireGuideFeature), so the fixture must carry
// them for a test to exercise anything past the gate.
func guideEnabledSettings() *conf.Settings {
	s := &conf.Settings{}
	s.Realtime.Dashboard.SpeciesGuide.Enabled = true
	s.Realtime.Dashboard.SpeciesGuide.ShowNotes = true
	s.Realtime.Dashboard.SpeciesGuide.ShowEnrichments = true
	s.Realtime.Dashboard.SpeciesGuide.ShowSimilarSpecies = true
	s.Realtime.Dashboard.SpeciesGuide.ShowTaxonomy = true
	return s
}
...
// notesHandler builds a handler with a mock datastore and an ENABLED guide: the notes
// endpoints are gated on Enabled + ShowNotes (see requireGuideFeature), so a
// zero-valued settings snapshot would 404 before reaching the datastore and every
// assertion below would be testing the gate rather than the handler.
func notesHandler(t *testing.T) (*Handler, *mocks.MockInterface) {
	t.Helper()
	mockDS := mocks.NewMockInterface(t)
	c := New(&apicore.Core{DS: mockDS})
	c.Settings.Store(guideEnabledSettings())
	return c, mockDS
}
...
func TestGetSpeciesNotes_DatastoreUnavailableReturns503(t *testing.T) {
	...
	// Guide enabled: the feature gate runs before RequireDatastore, so a zero-valued
	// snapshot would return 404 and never reach the datastore check under test.
	c.Settings.Store(guideEnabledSettings())
...
// --- feature gating (the show* flags are enforced server-side, not just in the UI) ---

// guideSettingsWith returns enabled-guide settings with one sub-feature switched off.
func guideSettingsWith(t *testing.T, off func(*conf.SpeciesGuideConfig)) *conf.Settings {
	t.Helper()
	s := guideEnabledSettings()
	off(&s.Realtime.Dashboard.SpeciesGuide)
	return s
}

// A disabled sub-feature must stop the endpoint serving, not merely stop the frontend
// rendering: otherwise a switched-off panel still costs the taxonomy lookup and the
// live guide fetches it was switched off to avoid.
func TestGetSimilarSpecies_ShowSimilarSpeciesOffReturns404(t *testing.T) {
	c := guideTestHandler(t, guideSettingsWith(t, func(cfg *conf.SpeciesGuideConfig) {
		cfg.ShowSimilarSpecies = false
	}))
	ctx, rec := guideCtx(t, sciEurasianBlackbird)
	require.NoError(t, c.GetSimilarSpecies(ctx))
	assert.Equal(t, http.StatusNotFound, rec.Code)
...
// All four notes endpoints honor ShowNotes and the top-level Enabled switch. Before
// this gating, disabling the species guide left its notes API fully writable, so the
// notes table kept growing for a feature the operator had turned off.
func TestSpeciesNotes_FeatureGating(t *testing.T) {
	cases := []struct {
		name string
		// off switches one sub-feature off on an otherwise-enabled guide; nil means
		// the whole guide is disabled via the top-level Enabled switch.
		off func(*conf.SpeciesGuideConfig)
	}{
		{"guide disabled", nil},
		{"notes disabled", func(cfg *conf.SpeciesGuideConfig) { cfg.ShowNotes = false }},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			settings := &conf.Settings{}
			if tc.off != nil {
				settings = guideSettingsWith(t, tc.off)
			}
			c := guideTestHandler(t, settings)

			ctx, rec := guideCtx(t, sciEurasianBlackbird)
			require.NoError(t, c.GetSpeciesNotes(ctx))
			assert.Equal(t, http.StatusNotFound, rec.Code, "GET notes")

			ctx, rec = notesCtx(t, http.MethodPost, "/api/v2/species/x/notes", `{"entry":"x"}`,
				map[string]string{paramScientificName: sciEurasianBlackbird})
			require.NoError(t, c.CreateSpeciesNote(ctx))
			assert.Equal(t, http.StatusNotFound, rec.Code, "POST notes")

			ctx, rec = notesCtx(t, http.MethodPut, "/api/v2/species/notes/1", `{"entry":"x"}`,
				map[string]string{"id": "1"})
			require.NoError(t, c.UpdateSpeciesNote(ctx))
			assert.Equal(t, http.StatusNotFound, rec.Code, "PUT note")

			ctx, rec = notesCtx(t, http.MethodDelete, "/api/v2/species/notes/1", "",
				map[string]string{"id": "1"})
			require.NoError(t, c.DeleteSpeciesNote(ctx))
			assert.Equal(t, http.StatusNotFound, rec.Code, "DELETE note")
		})
	}
}
