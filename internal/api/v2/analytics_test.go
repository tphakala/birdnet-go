...
func TestGetSpeciesSummary(t *testing.T) {
	...
	if assert.NoError(t, handler(c)) {
		...
		parsedFirst, parseErr := time.Parse(time.RFC3339, firstHeard)
		require.NoErrorf(t, parseErr, "first_heard %q must be valid RFC3339", firstHeard)
		...
		parsedLast, parseErr := time.Parse(time.RFC3339, lastHeard)
		require.NoErrorf(t, parseErr, "last_heard %q must be valid RFC3339", lastHeard)
