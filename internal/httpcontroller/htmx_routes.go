...
func (s *Server) initRoutes() {
	...
	// Support endpoints
	s.Echo.POST("/api/v1/support/generate", h.WithErrorHandling(h.GenerateSupportDump), s.AuthMiddleware)
	s.Echo.GET("/api/v1/support/download/:id", h.WithErrorHandling(h.DownloadSupportDump), s.AuthMiddleware)
	s.Echo.GET("/api/v1/support/status", h.WithErrorHandling(h.GetSupportStatus), s.AuthMiddleware)
