...
func Sources() map[string]Source {
	...
	//
	// In-package callers on the request path use sourcesReadOnly instead: they only
	// index the map, so the per-call copy is pure waste there.
	return maps.Clone(sourcesReadOnly())
}

// sourcesReadOnly returns the shared sources registry WITHOUT copying it, parsing it
// once. Callers must only read from it (index/range) and must never add, remove, or
// modify keys — it is the process-wide map every concurrent link resolution reads.
// Use this on the request path: ExternalLinks resolves once per species, and a
// similar-species response resolves up to maxSimilarSpecies + 1 times per request, so
// the clone Sources() performs would copy the whole registry for every species.
func sourcesReadOnly() map[string]Source {
...
// It returns the shared map WITHOUT copying it (the registry is parsed once and only
// ever read), so the sole caller — ExternalLinks, on the request path — does not pay a
// registry copy per species. Callers must treat it, and the Source values in it, as
// read-only.
