...
func NewHandlerChain() HandlerChain {
	...
	if l := logging.ForService("audiocore"); l != nil {
		logger = l.With("component", "detection_handler_chain")
	} else {
...
func (c *handlerChainImpl) HandleAnalysisResult(ctx context.Context, result *AnalysisResult) error {
	...
	// Process through handlers but don't return handler errors
	// This follows the pattern where the chain continues even if a handler fails
	for _, handler := range handlers {
