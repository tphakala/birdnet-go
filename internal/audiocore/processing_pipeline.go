...
func (p *ProcessingPipeline) processAnalysisResult(result *AnalysisResult, chunk *AudioData) {
	...
	// CRITICAL: Copy PCM data to transfer ownership to the queue consumer
	// We always allocate a new buffer here rather than using the buffer pool because:
	// 1. The birdnet.Results struct doesn't support buffer handles for proper release
	// 2. The queue consumer expects to own the data without managing buffer pools
	// 3. This maintains compatibility with the existing processor implementation
	//
	// TODO: For large buffers (>64KB), consider adding buffer pool support to birdnet.Results
	// This would require modifying the processor to handle buffer releases
	pcmDataCopy := make([]byte, len(chunk.Buffer))
	copy(pcmDataCopy, chunk.Buffer)
	
	// Create birdnet.Results struct matching the format from myaudio
	queueResult := birdnet.Results{
		StartTime:   result.Timestamp,
		ElapsedTime: elapsedTime,
		PCMdata:     pcmDataCopy,      // Copy of PCM data (ownership transfers to queue)
	...
	if p.metrics != nil && queueDepth > 0 {
		p.metrics.UpdateQueueDepth(queueDepth)
	}
	...
	case birdnet.ResultsQueue <- queueResult:
		...
		if p.metrics != nil {
			p.metrics.RecordQueueSubmission(result.SourceID, len(datastoreResults))
		}
	default:
