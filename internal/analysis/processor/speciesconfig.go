...
func LoadSpeciesConfig(fileName string) (SpeciesConfig, error) {
	var speciesConfig SpeciesConfig
	speciesConfig.Threshold = make(map[string]float32)
	speciesConfig.Actions = make(map[string]SpeciesActionConfig)
	...
	if file == nil {
		return SpeciesConfig{}, fmt.Errorf("file '%s' not found in default config paths", fileName)
	}
	defer file.Close()
	...
	if err != nil {
		...
		return SpeciesConfig{}, fmt.Errorf("error reading CSV file '%s': %w", fileName, err)
	}
	...
	for _, record := range records {
		...
		speciesConfig.Actions[species] = SpeciesActionConfig{
			SpeciesName: species,
			Actions:     actions,
			Exclude:     exclude,
...
func parseActions(actionStr string) (actions []ActionConfig, exclude []string, onlyActions bool) {
...
func (p *Processor) createActionsFromConfig(speciesConfig SpeciesActionConfig, detection Detections) []Action {
	var actions []Action
