...
func registerSoundLevelProcessorsForActiveSources(settings *conf.Settings) error {
	...
	for _, stream := range settings.Realtime.RTSP.GetStreamConfigs() {
		...
		registry := myaudio.GetRegistry()
		audioSource := registry.GetOrCreateSource(stream.URL, myaudio.SourceTypeRTSP)
		if audioSource == nil {
			errs = append(errs, errors.Newf("failed to get/create RTSP source").
				...
				Build())
			LogSoundLevelProcessorRegistrationFailed(privacy.SanitizeRTSPUrl(stream.URL), "rtsp", "analysis.soundlevel", fmt.Errorf("failed to get/create RTSP source"))
			continue
		}
...
func unregisterAllSoundLevelProcessors(settings *conf.Settings) {
	...
	// Unregister all RTSP sources - use registry to get consistent IDs
	for _, stream := range settings.Realtime.RTSP.GetStreamConfigs() {
		if stream.URL == "" {
			continue
		}
		registry := myaudio.GetRegistry()
		if registry != nil {
			if audioSource := registry.GetOrCreateSource(stream.URL, myaudio.SourceTypeRTSP); audioSource != nil {
				myaudio.UnregisterSoundLevelProcessor(audioSource.ID)
				LogSoundLevelProcessorUnregistered(audioSource.DisplayName, "rtsp_stream", "analysis.soundlevel")
			} else {
				GetLogger().Warn("failed to get RTSP source from registry during sound level processor unregistration",
					logger.String("url", privacy.SanitizeRTSPUrl(stream.URL)))
			}
		} else {
			GetLogger().Warn("registry not available during RTSP sound level processor unregistration")
		}
	}
}
