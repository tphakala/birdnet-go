...
- [ ] Linters clean: task lint (golangci-lint over the whole module) and npm run check:all pass (zero warnings)
- [ ] Tests pass: task test (go test -race) and npm test pass
- [ ] Tagged or OS-specific Go files changed: linted with those build tags and built for that platform (see the root AGENTS.md)
