...
<button
  ...
  onmouseenter={(e) => !disabled && !loading && (e.currentTarget.style.backgroundColor = 'color-mix(in oklab, var(--fallback-p, oklch(var(--p)/1)) 90%, black)')}
  onmouseleave={(e) => !disabled && !loading && (e.currentTarget.style.backgroundColor = 'var(--fallback-p, oklch(var(--p)/1))')}
