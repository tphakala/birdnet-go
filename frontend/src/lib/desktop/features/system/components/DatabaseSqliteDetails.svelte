...
<div class="bg-[var(--surface-100)] border border-[var(--border-100)] rounded-xl p-4 shadow-sm">
  ...
  <div class="space-y-2.5">
    <div class="flex items-center gap-3">
      <Database class="w-3.5 h-3.5 flex-shrink-0 text-muted" />
      <div class="flex justify-between flex-1 text-sm">
        <span class="text-muted">{t('system.database.dashboard.details.engine')}</span>
        <span class="font-medium">{details.engine_version}</span>
      </div>
    </div>
    <div class="flex items-center gap-3">
      <FileText class="w-3.5 h-3.5 flex-shrink-0 text-muted" />
      <div class="flex justify-between flex-1 text-sm">
        <span class="text-muted">{t('system.database.dashboard.details.journal')}</span>
        <span class="font-medium uppercase">{details.journal_mode}</span>
      </div>
    </div>
    <div class="flex items-center gap-3">
      <HardDrive class="w-3.5 h-3.5 flex-shrink-0 text-muted" />
      <div class="flex justify-between flex-1 text-sm">
        <span class="text-muted">{t('system.database.dashboard.details.pageSize')}</span>
    ...
    <div class="flex items-center gap-3">
      <Shield class="w-3.5 h-3.5 flex-shrink-0 text-muted" />
      <div class="flex justify-between flex-1 text-sm">
        <span class="text-muted">{t('system.database.dashboard.details.integrity')}</span>
        <span
          class="font-medium {details.integrity_check === 'ok'
            ? 'text-[var(--color-success)]'
            : 'text-[var(--color-error)]'}">{details.integrity_check.toUpperCase()}</span
        >
      </div>
    </div>
    <div class="flex items-center gap-3">
      ...
      <div class="flex justify-between flex-1 text-sm">
        <span class="text-muted">{t('system.database.dashboard.details.lastVacuum')}</span>
