...
<div class="bg-[var(--surface-100)] border border-[var(--border-100)] rounded-xl p-4 shadow-sm">
  ...
  <div class="space-y-2.5">
    <div class="flex items-center gap-3">
      <Database class="w-3.5 h-3.5 flex-shrink-0 text-slate-600 dark:text-slate-400" />
      <span class="text-sm">{stats ? stats.type.toUpperCase() : 'SQLite'}</span>
    </div>
    <div class="flex items-center gap-3">
      <MapPin class="w-3.5 h-3.5 flex-shrink-0 text-slate-600 dark:text-slate-400" />
      <span class="text-sm font-mono truncate" title={stats?.location ?? ''}
        >{stats?.location ?? '—'}</span
      >
    </div>
    <div class="flex items-center gap-3">
      <HardDrive class="w-3.5 h-3.5 flex-shrink-0 text-slate-600 dark:text-slate-400" />
      <span class="text-sm tabular-nums">{stats ? formatBytesCompact(stats.size_bytes) : '—'}</span>
    </div>
    <div class="flex items-center gap-3">
      <FileText class="w-3.5 h-3.5 flex-shrink-0 text-slate-600 dark:text-slate-400" />
    ...
    <div class="flex items-center gap-3">
      <Shield class="w-3.5 h-3.5 flex-shrink-0 text-slate-600 dark:text-slate-400" />
      <span class="text-sm"
