...
4.  **Install Dependencies:**
    - **FFmpeg:** Required for RTSP stream capture, audio export to MP3, AAC, and Opus (WAV and FLAC are encoded natively and do not need FFmpeg), and the [Live Audio Streaming](guide.md#live-audio-streaming) feature. Native AAC and Opus encoders are also available as an opt-in preview via the `BIRDNET_AAC_ENCODER=native` and `BIRDNET_OPUS_ENCODER=native` environment variables; FFmpeg remains the default for both. Install using your system's package manager (e.g., `sudo apt install ffmpeg` on Debian/Ubuntu, `brew install ffmpeg` on macOS).
