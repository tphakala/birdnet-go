...
- **FFmpeg**: Required for RTSP stream capture, audio export to MP3, AAC, and Opus (WAV and FLAC are encoded natively and do not need FFmpeg), and for the HLS live stream feature in the web interface. Native AAC and Opus encoders are available as an opt-in preview via the `BIRDNET_AAC_ENCODER=native` and `BIRDNET_OPUS_ENCODER=native` environment variables; FFmpeg remains the default for both.
