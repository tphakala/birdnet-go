...
* MQTT support for IoT ecosystems.
  ...
  - Turning Home Assistant discovery off removes BirdNET-Go's entities from Home Assistant. Deleting a stream in settings removes that stream's entities. Renaming the node (main name) or changing the discovery prefix moves the entities: the old ones are removed and republished under the new identity. A source that merely restarts (watchdog, quiet hours, a brief unplug) keeps its entities and retained state. Turning sound level monitoring off removes the Sound Level sensor.
